...
```jsx
switch(조건식) { 
case 비교조건식 : 동작문 } //조건식 & 비교조건식 둘 다 true여야 동작문사용함.
```

조건부 연산자

조건식 ? 참일 때 실행하는 식 : 거짓일 때 실행되는 식
...
⇒ 중첩 조건문은 행을 더 길게 만들어 여러 행으로 분할하도록 해야하고, 조건의 중첩에 따라 테스트케이스의 수 증가, 읽기 어렵게 만들기 때문에 사용을 지향하는 것이 좋다.
